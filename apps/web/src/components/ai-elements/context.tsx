...
export const Context = ({ usedTokens, maxTokens, usage, modelId, showCost = false, ...props }: ContextProps) => (
...
export const ContextContentFooter = ({ children, className, ...props }: ContextContentFooterProps) => {
  const { modelId, usage, showCost } = useContextValue();

  if (!showCost && !children) {
    return null;
  }
...
export const ContextInputUsage = ({ className, children, ...props }: ContextInputUsageProps) => {
  const { usage, modelId, showCost } = useContextValue();
...
export const ContextReasoningUsage = ({ className, children, ...props }: ContextReasoningUsageProps) => {
  ...
  const reasoningCost =
    showCost && modelId
      ...
      : undefined;
  const reasoningCostText = showCost
    ? new Intl.NumberFormat("en-US", {
        style: "currency",
        currency: "USD",
